...
//--------------------------------------------------------------------------------------------------
void runSkimMonoJet(const char *fileset    = "0000",
		    ...
		    int         nEvents    = 1000)
{
 ...
 JetCorrectionMod *jetCorr = new JetCorrectionMod;
  if (isData){ 
    jetCorr->AddCorrectionFromFile(MitData + "/Summer13_V1_DATA_L1FastJet_AK5PF.txt"); 
    jetCorr->AddCorrectionFromFile(MitData + "/Summer13_V1_DATA_L2Relative_AK5PF.txt"); 
    jetCorr->AddCorrectionFromFile(MitData + "/Summer13_V1_DATA_L3Absolute_AK5PF.txt"); 
    //    jetCorr->AddCorrectionFromFile(MitData + "/Summer13_V1_DATA_L2L3Residual_AK5PF.txt");
  }                                                                                      
  else {                                                                                 
  ...
  jetCorr->SetRhoAlgo(PileupEnergyDensity::kHighEta);
  ...
  // reusing the jet corrector core of JetCorrectionMod
  // if jet collections are different between the two
  // (e.g. when JetCol uses CHS but MET correction is standard jets)
  // then one needs to set up corrections using metCorrT0T1Shift->AddJetCorrectionFromFile()
  metCorrT0T1Shift->SetJetCorrector(jetCorr->GetCorrector());
  metCorrT0T1Shift->SetRhoAlgo(PileupEnergyDensity::kHighEta);
  ...
  if (TString(skim).CompareTo("noskim") == 0)
    d = c->FindDataset(book,dataset,fileset,local);
  else
    d = c->FindDataset(book,skimDataset.Data(),fileset,local);
  ana->AddDataset(d);
  ana->SetCacheSize(0);
  ...
  // Make sure skim module gets added in the end
